...
public class OrderController  extends BaseController{
	...
	public String addOrder(String receiverinfo,String cartIds,HttpSession session) {
		...
		//Orders myOrder = new Orders(null,null,receiverinfo,0,timestamp,user.getId());
	...
	public String showorder(HttpSession session,Model model) {
		...
		//3.将该用户的所有订单信息的list集合存入到request域中，转发d到order_list。jsp中显示
	...
	private List<OrderInfo> findOrderInfoByUserId(int userId) {
		// TODO Auto-generated method stub
		 List<OrderInfo> orderInfoList = new ArrayList<OrderInfo>();
		 //1.根据用户id查询该用户的所有订单信息，查询orders表
		 List<Orders> orderList = orderService.findOrderByUserId(userId);
		 //2.便利每一个订单，通过订单id查询当前订单中包含的所有订单信息
		 for(Orders order : orderList) {
			 //根据用户order_id查询该订单号的所有订单信息，查询orderitem表
